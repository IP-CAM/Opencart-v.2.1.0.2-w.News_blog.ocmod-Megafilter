...
class ControllerCommonSeoPro extends Controller {
    ...
    public function index() {
        ...
        $code = null;
        ////////My  index pagination , Розпарсює урл достає пагінацію
        ...
        $parts = explode('/', $route);
        $parts_end = end($parts);
        if (strpos($parts_end, 'page-') !== false AND ($parts_end!='page-0') AND ($parts_end!='page-')) {

            list($key, $value) = explode("-", $parts_end);


            $this->request->get[$key] = $value;
            $this->request->get['_route_'] = str_replace($key . '-' . $value, '', $route);
...
//var_export($_REQUEST);
    ...
    public function rewrite($link, $code = '') {
        ...
        switch ($route) {
            ...
            ################ncat seo##########################
            case 'news/ncategory':
                //TODO get real uniqui blog path
        ...
        foreach ($data as $key => $value) {
            switch ($key) {
                ...
                ################ncat seo##########################
                case 'ncat':
                    $ncategory_path = explode('_', $value);
                   //TODO get real uniqui blog path
                    foreach ($ncategory_path as $category) {
                        $queries[] = $key . '=' . $category;
                    }
                    unset($data[$key]);
                    //   $postfix = 1;
                    break;
        ...
        foreach($queries as $query) {
            ...
            if(preg_match('/^page=/', $query) === 1 && $query != 'page=1') {
                $rows[] = ['query' => $query, 'keyword' => isset($this->cache_data['queries'][$route])? $this->cache_data['queries'][$route]: ''];
            }
        }
        ...
        ////////My rewrite pagination , переписує пагінацію BEGINvv
        if ($data) {
            $data2 = $data;
        ...
        ////////My rewrite pagination , переписує пагінацію END
